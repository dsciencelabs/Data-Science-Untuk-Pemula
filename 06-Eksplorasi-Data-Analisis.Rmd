...
Pada bagian ini, kita menggunakan dataset Students, download dan simpan di komputer Anda (PC).
...
```{r, echo=T, results='hide'}
# lakukan untuk setiap variabel kategori
```
...
$\bar{x}=\frac{(x_1+x_2+...+x_n)}{n}$, atau
$\bar{x}=\frac{\Sigma{x}}{n}$, atau
$\mu=\frac{\Sigma{x}}{n}$
...
## 6.3 ADE dengan cara Malas (Lazy-way)
